...
from ..models.action_items import (
    ActionItem,
    ActionItemStatus,
    ActionItemPriority,
    ProcessingLog,
...
async def list_action_items(
    ...
    priority: Optional[ActionItemPriority] = Query(
    ...
    items = (
        query.order_by(desc(ActionItem.created_at))
...
async def get_action_item_stats(
    ...
    in_progress_items = sum(
        1 for item in items if item.status == ActionItemStatus.IN_PROGRESS
    )
    completed_items = sum(
        1 for item in items if item.status == ActionItemStatus.COMPLETED
    )
    cancelled_items = sum(
        1 for item in items if item.status == ActionItemStatus.CANCELLED
    )

    high_priority_items = sum(
        1
        for item in items
        if item.priority in [ActionItemPriority.HIGH, ActionItemPriority.URGENT]
    )
    items_with_deadlines = sum(1 for item in items if item.deadline)

    # Average confidence
    avg_confidence = (
        sum(item.confidence for item in items) / total_items if total_items > 0 else 0.0
    )
...
async def bulk_update_action_items(
    ...
    for item in items:
        ...
        if (
            update_data.status == ActionItemStatus.COMPLETED
