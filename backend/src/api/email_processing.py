...
class EmailProcessingRequest(BaseModel):
    days_back: int = Field(
        7, description="Number of days to look back for emails", ge=1, le=30
    )
    max_emails: int = Field(
        20, description="Maximum number of emails to process", ge=1, le=100
    )
...
async def get_email_folders(
    ...
    try:
        ...
        return EmailFoldersResponse(status="success", folders=folders)

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get email folders: {str(e)}"
        )
...
async def process_emails_for_actions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to process emails for actions: {str(e)}"
        )
...
async def get_email_content(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get email content: {str(e)}"
        )
...
async def test_email_connection(
    ...
    try:
        ...
        emails = await processor.get_recent_emails(
            db_session=db_session, days_back=1, max_emails=5, filter_unread=False
        )
