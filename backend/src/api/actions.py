...
async def get_action_items(
    ...
    items = (
        query.order_by(desc(ActionItemDB.created_at))
        .offset((page - 1) * size)
        .limit(size)
        .all()
    )
...
async def get_pending_action_items(db: Session = Depends(get_database)):
    ...
    items = (
        db.query(ActionItemDB)
        .filter(ActionItemDB.status == ActionItemStatus.PENDING)
        .order_by(desc(ActionItemDB.confidence_score), desc(ActionItemDB.created_at))
        .all()
    )
...
async def get_processing_stats(db: Session = Depends(get_database)):
    ...
    action_items_today = (
        db.query(ActionItemDB)
        .filter(
            and_(ActionItemDB.created_at >= today, ActionItemDB.created_at < tomorrow)
        )
        .count()
    )
...
async def update_action_item(
    item_id: int, update_data: ActionItemUpdate, db: Session = Depends(get_database)
):
...
async def approve_action_item(item_id: int, db: Session = Depends(get_database)):
    ...
    try:
        ...
        return {
            "message": "Action item approved and Wrike task created",
            "wrike_task_id": wrike_task.get("id"),
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create Wrike task: {str(e)}"
        )
...
async def batch_approve_action_items(
    item_ids: List[int], db: Session = Depends(get_database)
):
    ...
    for item_id in item_ids:
        try:
            ...
            # Create Wrike task
            wrike = WrikeIntegrator()
            wrike_task = await wrike.create_task_from_action_item(
                ActionItem.from_orm(item)
            )
