...
class MSGraphAuth:
    def __init__(self):
        ...
        self.tenant_id = (
            self.settings.microsoft_tenant_id or "common"
    ...
    def _store_oauth_state(
        self, state: str, code_verifier: str, scopes: List[str]
    ) -> None:
    ...
    def get_auth_url(
        self, custom_scopes: Optional[List[str]] = None
    ) -> tuple[str, str, str]:
    ...
    async def exchange_code_for_token(
        self, code: str, state: str, code_verifier: str = None
    ) -> Dict[str, Any]:
        ...
        result = self.app.acquire_token_by_authorization_code(
            code=code, scopes=state_data["scopes"], redirect_uri=self.redirect_uri
        )
    ...
    async def refresh_token(self, refresh_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if "error" in result:
                error_desc = result.get("error_description", "Unknown error")
                raise HTTPException(
                    status_code=401, detail=f"Token refresh failed: {error_desc}"
                )
            ...
            raise HTTPException(
                status_code=401, detail=f"Failed to refresh Microsoft token: {str(e)}"
            )
...
async def auth_status(db_session=Depends(get_db_session)):
    ...
    try:
        ...
        if wrike_authenticated:
            ...
            try:
                wrike_user = {
                    ...
                    "firstName": (
                        wrike_token.user_name.split(" ")[0]
                        if wrike_token.user_name
                        else None
                    ),
                    "lastName": (
                        " ".join(wrike_token.user_name.split(" ")[1:])
                        if wrike_token.user_name and " " in wrike_token.user_name
                        else None
                    ),
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    response = await client.get(
                        "https://www.wrike.com/api/v4/contacts?me=true", headers=headers
                    )
                    if response.status_code != 200:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to check auth status: {str(e)}"
        )
...
async def microsoft_admin_consent():
    ...
    try:
        if not ms_auth:
            raise HTTPException(
                status_code=500, detail="Microsoft Graph authentication not configured"
            )
        ...
        admin_consent_url = f"{ms_auth.authority}/adminconsent?" + "&".join(
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate admin consent URL: {str(e)}"
        )
...
async def microsoft_login(scopes: Optional[str] = None):
    ...
    try:
        ...
        return AuthUrlResponse(
            auth_url=auth_url, state=state, code_verifier=code_verifier
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate Microsoft auth URL: {str(e)}"
        )


@router.post("/microsoft/callback")
async def microsoft_callback(
    callback_data: AuthCallbackRequest, db_session=Depends(get_db_session)
):
    ...
    try:
        ...
        return {
            ...
            "user": {
                "displayName": (
                    user_info.get("displayName") if user_info else "Unknown User"
                ),
                "email": (
            ...
            "token_expires_at": (
                stored_token.expires_at.isoformat() if stored_token.expires_at else None
            ),
        }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to complete Microsoft authentication: {str(e)}",
        )
...
async def wrike_login():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate Wrike auth URL: {str(e)}"
        )
...
async def wrike_callback(
    ...
    if error:
        raise HTTPException(
            status_code=400, detail=f"Wrike OAuth error: {error_description or error}"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to complete Wrike authentication: {str(e)}"
        )
...
async def microsoft_logout(db_session=Depends(get_db_session)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to revoke Microsoft authentication: {str(e)}",
        )
...
async def wrike_logout(db_session=Depends(get_db_session)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to revoke Wrike authentication: {str(e)}"
        )


@router.post("/test-connection")
async def test_connection(provider: str, db_session=Depends(get_db_session)):
    """Test API connection for a provider"""

    if provider not in ["microsoft", "wrike"]:
        raise HTTPException(
            status_code=400, detail="Invalid provider. Must be 'microsoft' or 'wrike'"
        )

    try:
        token_provider = (
            TokenProvider.MICROSOFT if provider == "microsoft" else TokenProvider.WRIKE
        )
        token = await AuthToken.get_valid_token(db_session, token_provider)

        if not token:
            raise HTTPException(
                status_code=401, detail=f"No valid {provider} token found"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to test {provider} connection: {str(e)}"
        )
...
async def get_tokens(db_session=Depends(get_db_session)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get token information: {str(e)}"
        )
