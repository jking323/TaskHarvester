...
class SyncActionItemsRequest(BaseModel):
    action_item_ids: Optional[List[int]] = Field(
        None, description="Specific action item IDs to sync"
    )
...
async def get_sync_candidates(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get sync candidates: {str(e)}"
        )


@router.post("/sync-to-wrike", response_model=SyncActionItemsResponse)
async def sync_action_items_to_wrike(
    request: SyncActionItemsRequest, db: Session = Depends(get_database)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to sync action items to Wrike: {str(e)}"
        )
...
async def get_wrike_integration_status(db: Session = Depends(get_database)):
    ...
    synced_items = (
        db.query(ActionItem).filter(ActionItem.wrike_task_id.isnot(None)).count()
    )
    unsynced_items = total_items - synced_items

    # Get high-confidence unsynced items
    high_confidence_unsynced = (
        db.query(ActionItem)
    ...
    return {
        ...
        "sync_rate": (
            round((synced_items / total_items * 100), 1) if total_items > 0 else 0
        ),
    }
