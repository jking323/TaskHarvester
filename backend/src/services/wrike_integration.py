...
class WrikeIntegration:
    ...
    async def create_task_from_action_item(
        self, action_item: ActionItem, db_session, folder_id: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        print(
            f"[WRIKE] Simulated task creation for action item {action_item.id}: '{action_item.task[:50]}...'"
        )
    ...
    async def sync_action_items_to_wrike(
        ...
        for action_item in action_items:
            try:
                ...
                failed_count += 1
                print(
                    f"[ERROR] Failed to sync action item {action_item.id} to Wrike: {e}"
                )
    ...
    async def get_sync_candidates(
        self, db_session, confidence_threshold: float = 0.7, days_back: int = 7
    ) -> List[Dict[str, Any]]:
