...
Extract action items (JSON only):"""
    ...
    async def extract_action_items(self, content: str, source_type: str = "email", source_id: str = "") -> List[ActionItem]:
        ...
        action_keywords = [
            'action required', 'follow up', 'deadline', 'due date', 'please',
            'need to', 'should', 'must', 'will you', 'can you', 'todo', 'task',
            'assignment', 'complete', 'deliver', 'prepare', 'schedule', 'meeting',
            'review', 'send', 'submit', 'finish', 'update'
        ]
        
        content_lower = content.lower()
        keyword_count = sum(1 for keyword in action_keywords if keyword in content_lower)
        ...
        try:
            ...
            # Parse response
            action_items = self._parse_llm_response(response['response'], source_type, source_id)
    ...
    async def test_extraction(self, test_content: str = None) -> Dict[str, any]:
        ...
        try:
            action_items = await self.extract_action_items(test_content, "test_email", "test_001")
