...
class BackgroundMonitor:
    ...
    async def _run_monitoring_cycle(self):
        ...
        if (
            self.last_email_check is None
            or (now - self.last_email_check).total_seconds()
            >= self.process_emails_interval
        ):
        ...
        if (
            self.last_wrike_sync is None
            or (now - self.last_wrike_sync).total_seconds() >= self.sync_wrike_interval
        ):
    ...
    async def _process_recent_emails(self):
        ...
        try:
            ...
            processing_result = {"emails_processed": 0, "action_items_found": 0}

            # Simulate email processing
            print(
                f"[MONITOR] Processed {processing_result['emails_processed']} emails, "
    ...
    def get_monitoring_stats(self) -> Dict[str, Any]:
        ...
        if self.stats["monitoring_started"]:
            ...
            current_stats["monitoring_started"] = self.stats[
                "monitoring_started"
            ].isoformat()

        current_stats["is_running"] = self.is_running
        current_stats["last_email_check"] = (
            self.last_email_check.isoformat() if self.last_email_check else None
        )
        current_stats["last_wrike_sync"] = (
            self.last_wrike_sync.isoformat() if self.last_wrike_sync else None
        )
